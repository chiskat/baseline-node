...
export NODE_VERSION="$(latest_node_version 24)-slim"
...
if ! docker manifest inspect "chiskat/baseline-node24:$NODE_NPM_VERSION" >/dev/null 2>&1; then
  source "$REPO_ROOT/scripts/docker_build.sh"
  source "$REPO_ROOT/scripts/docker_push.sh"

  docker_build "baseline-node24" "$REPO_ROOT/docker/node-corepack.Dockerfile"
  docker_push "chiskat/baseline-node24"
fi

source "$REPO_ROOT/scripts/docker_readme.sh"
docker_readme "chiskat/baseline-node24" "Node.js v$NODE_VERSION (slim) and pnpm@$PNPM_VERSION are preinstalled." "$REPO_ROOT/24/baseline-node24/README.md"
