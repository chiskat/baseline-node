...
export NODE_VERSION="$(latest_node_version 24)-alpine"
...
if ! docker manifest inspect "chiskat/baseline-node24-alpine:$NODE_NPM_VERSION" >/dev/null 2>&1; then
  source "$REPO_ROOT/scripts/docker_build.sh"
  source "$REPO_ROOT/scripts/docker_push.sh"

  docker_build "baseline-node24-alpine" "$REPO_ROOT/docker/node-corepack-alpine.Dockerfile"
  docker_push "chiskat/baseline-node24-alpine"
fi

source "$REPO_ROOT/scripts/docker_readme.sh"
docker_readme "chiskat/baseline-node24-alpine" "Node.js v$NODE_VERSION (alpine) with pnpm@$PNPM_VERSION, alpine but compatible with Next.js" "$REPO_ROOT/24/baseline-node24-alpine/README.md"
