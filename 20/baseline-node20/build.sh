...
if ! docker manifest inspect "paperplanecc/baseline-node20:$NODE_NPM_VERSION" > /dev/null 2>&1; then
  ...
  docker_push "paperplanecc/baseline-node20"
fi

source "/repo/scripts/docker_readme.sh"
docker_readme "paperplanecc/baseline-node20" "Node.js v$NODE_VERSION (slim) and pnpm@$PNPM_VERSION are preinstalled." "/repo/20/baseline-node20/README.md"