...
if ! docker manifest inspect "paperplanecc/baseline-node20-alpine:$NODE_NPM_VERSION" > /dev/null 2>&1; then
  ...
  docker_push "paperplanecc/baseline-node20-alpine"
fi

source "/repo/scripts/docker_readme.sh"
docker_readme "paperplanecc/baseline-node20-alpine" "Node.js v$NODE_VERSION (alpine) with pnpm@$PNPM_VERSION, alpine but compatible with Next.js" "/repo/20/baseline-node20-alpine/README.md"