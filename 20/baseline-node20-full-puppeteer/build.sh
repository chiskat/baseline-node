...
if docker manifest inspect "chiskat/baseline-node20-full:$CALVER" >/dev/null 2>&1; then
  ...
  docker_build_puppeteer "chiskat/baseline-node20-full-puppeteer" "chiskat/baseline-node20-full:$CALVER" "$REPO_ROOT/docker/puppeteer.Dockerfile"
  docker_push "chiskat/baseline-node20-full-puppeteer"
fi

source "$REPO_ROOT/scripts/docker_readme.sh"
docker_readme "chiskat/baseline-node20-full-puppeteer" "Node.js, pnpm and Chromium are preinstalled, prepared for puppeteer." "$REPO_ROOT/20/baseline-node20-full-puppeteer/README.md"
