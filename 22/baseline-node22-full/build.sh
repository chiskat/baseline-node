...
export NODE_VERSION=$(latest_node_version 22)
...
if ! docker manifest inspect "chiskat/baseline-node22-full:$NODE_NPM_VERSION" >/dev/null 2>&1; then
  source "$REPO_ROOT/scripts/docker_build.sh"
  source "$REPO_ROOT/scripts/docker_push.sh"

  docker_build "baseline-node22-full" "$REPO_ROOT/docker/node-corepack.Dockerfile"
  docker_push "chiskat/baseline-node22-full"
fi

source "$REPO_ROOT/scripts/docker_readme.sh"
docker_readme "chiskat/baseline-node22-full" "Node.js v$NODE_VERSION and pnpm@$PNPM_VERSION are preinstalled." "$REPO_ROOT/22/baseline-node22-full/README.md"
